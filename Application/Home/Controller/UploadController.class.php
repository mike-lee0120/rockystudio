...
class UploadController extends HomeController
{
    ...
    public function upload()
    {
        if (is_login() || (C('AUTH_KEY') === $_SERVER['HTTP_UPLOADTOKEN'])) {
            ...
            // 如果是跨域上传需要回调处理
            if ($_SERVER['HTTP_ORIGIN'] !== request()->domain() && $_GET['callback_type'] && !C('IS_API')) {
                switch ($_GET['callback_type']) {
                    case 'kindeditor':
                        redirect($_SERVER['HTTP_ORIGIN'] . __ROOT__ . '/Public/libs/kindeditor/callback.php?json=' . $return);
                        break;
                    case 'editormd':
                        redirect($_SERVER['HTTP_ORIGIN'] . __ROOT__ . '/Public/libs/editormd/callback.php?json=' . $return);
                        break;
                    case 'summernote':
                        exit($return);
                        //redirect($_SERVER['HTTP_ORIGIN'] . __ROOT__ . '/Public/libs/summernote/callback.php?json=' . $return);
                        break;
                    default :
                        exit('跨域上传回调类型不存在');
                        break;
