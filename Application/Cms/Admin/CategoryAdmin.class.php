...
EOF;
    ...
    public function add($group = 1)
    {
        if (request()->isPost()) {
            ...
            // 获取前台模版供选择
            if (C('CURRENT_THEME')) {
                $template_list = \lyf\File::get_dirs(getcwd() . '/Theme/' . C('CURRENT_THEME') . '/' . D('Index')->moduleName . '/Index');
            } else {
                $template_list = \lyf\File::get_dirs(getcwd() . '/Application/' . D('Index')->moduleName . '/View/Index');
            }
    ...
    public function edit_with_tree($id, $group = 1)
    {
        if (request()->isPost()) {
            ...
            // 获取前台模版供选择
            if (C('CURRENT_THEME')) {
                $template_list = \lyf\File::get_dirs(getcwd() . '/Theme/' . C('CURRENT_THEME') . '/Cms/Index');
            } else {
                $template_list = \lyf\File::get_dirs(getcwd() . '/Application/Cms/View/Index');
            }
            foreach ($template_list['file'] as $val) {
                $val = substr($val, 0, -5);
                if (strstr($val, 'index')) {
