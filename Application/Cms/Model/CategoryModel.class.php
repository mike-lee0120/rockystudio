...
class CategoryModel extends Model
{
    ...
    {
        return \lyf\Str::parseAttr(C('cms_config.group_list'));
    }
    ...
    public function getParentCategory($cid, $group = 1)
    {
        ...
        $current_category = $this->field('id,pid,group,title,url,doc_type')->find($cid); //获取当前分类的信息
     ...
     * @author jry <598821125@qq.com>
     */
    public function getCategoryTree($id = 0, $limit = null, $page = 1, $group = 1, $field = true, $ignore_system = 0)
    {
        //获取当前分类信息
        if ((int) $id > 0) {
            $info = $this->find($id);
        ...
        if ($ignore_system) {
            // 获取系统模型
            $system_type_ids = D($this->moduleName . '/Type')->where('system = 1')->getField('id', true);
            if ($system_type_ids) {
                $map['doc_type'] = array('not in', $system_type_ids);
            }
        }
        $tree = new \lyf\Tree();
