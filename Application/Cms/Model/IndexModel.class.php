...
class IndexModel extends Model
{
    ...
    protected function _after_find(&$result, $options)
    {
        ...
        if ($result['images']) {
            ...
            foreach ($result['images'] as $image) {
                $result['images_url'][] = get_cover($image, 'default');
            }
        }
        if ($result['tags']) {
     ...
     * @author jry <598821125@qq.com>
     */
    public function getReviewStatus($id = null)
    {
        $list[0]  = '<span class="label label-warning">未审核</span>';
        $list[1]  = '<span class="label label-success">通过</span>';
        $list[-1] = '<span class="label label-danger">拒绝</span>';
            ...
            if ($extend_data) {
                if (empty($base_data['id'])) {
                    ...
                    if (!$_POST['source']) {
                        ...
                        // 前台投稿根据门户模块的配置决定是设为1还是设为-2
                        if (C('cms_config.need_check')) {
                            $base_data['status'] = '-2';
                        } else {
                            $base_data['status']         = '1';
                            $base_data['review_time']    = time();
                            $base_data['review_message'] = '自动通过';
                            $base_data['review_status']  = '1';
                        }
                    }

                    //新增基础内容
                    $base_id = $this->add($base_data);
                    if ($base_id) {
    ...
    public function getList($cid, $limit = 10, $page = 1, $order = null, $child = false, $map = null)
    {
        ...
        if (!$order) {
            $order = 'sort desc,create_time desc,' . $base_table . '.id desc';
        }
        ...
        foreach ($return_list as $key => &$val) {
            ...
            $val['title_url'] = '<a target="_blank" href="' . oc_url($this->moduleName . '/Index/detail', array('id' => $val['id'])) . '">' . $val[$main_field_name] . '</a>';
        }
        return $return_list;
    }
    ...
    public function detail($id, $map = null)
    {
        ...
        // 获取收藏状态
        $info['mark_status'] = D($this->moduleName . '/Mark')->get_status($info['id']);

        // 获取点赞状态
        $info['zan_status'] = D($this->moduleName . '/Zan')->get_status($info['id']);
        ...
        foreach ($filter_field_list as $key => $val) {
            $val['options']                      = \lyf\Str::parseAttr($val['options']);
        ...
        if ($info['file']) {
            ...
            foreach ($file_list as &$file) {
                ...
                $uid  = is_login();
                if ($uid) {
                    $file['token'] = \lyf\Crypt::encrypt($file['md5'], user_md5($uid), 3600);
                } else {
