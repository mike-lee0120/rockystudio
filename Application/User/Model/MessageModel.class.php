...
class MessageModel extends Model
{
    ...
    protected $_validate = array(
        ...
        array('to_uid', 'require', '收信人必须填写', self::MUST_VALIDATE, 'regex', self::MODEL_INSERT),
    );
    ...
    public function sendMessage($send_data, $send_type = array('message', 'email', 'weixin', 'push'))
    {
        ...
        if ($data) {
            ...
            if (in_array('push', $send_type) && (D('Admin/Addon')->where('name="Jpush" and status="1"')->count())) {
                // 获取用户对应的设备识别
                $token_list = D('User/MessagePush')->where(array('uid' => $send_data['to_uid']))->getField('token', true);
                if ($token_list) {
                    $send_data['token_list'] = $token_list;
                    $send_data['extras']     = $send_data;
            ...
            // 微信消息通知
            if (in_array('weixin', $send_type) && (D('Admin/Module')->where('name="Weixin" and status="1"')->count())) {
