 ...
 * @author jry <598821125@qq.com>
 */
class NavAdmin extends AdminController
{
    /**
     * 导航列表
     * @author jry <598821125@qq.com>
    ...
    public function index($group = 'main')
    {
        ...
        foreach ($data_list as &$val) {
            if ($val['type'] == 'post') {
                $val['title'] = '<a href="' . U('Forum/Post/index', array('cid' => $val['id'])) . '">' . $val['title'] . '</a>';
            }
        }
...
EOF;
    ...
    public function add($group)
    {
        if (request()->isPost()) {
            ...
            // 获取前台模版供选择
            if (C('CURRENT_THEME')) {
                $template_list = \lyf\File::get_dirs(getcwd() . '/Theme/' . C('CURRENT_THEME') . '/Forum//Nav');
            } else {
                $template_list = \lyf\File::get_dirs(getcwd() . '/Application/Forum/View/Nav');
            }
            ...
            $builder->setMetaTitle('新增导航') // 设置页面标题
                ...
                ->addFormItem('pid', 'select', '上级导航', '上级导航', select_list_as_tree('Forum/Nav', array('group' => $group), '顶级导航'))
                ->addFormItem('title', 'text', '导航标题', '导航前台显示标题')
                ->addFormItem('type', 'radio', '导航类型', '导航类型', D('Forum/Nav')->nav_type())
