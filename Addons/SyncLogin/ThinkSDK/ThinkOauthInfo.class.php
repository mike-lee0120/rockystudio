...
class ThinkOauthInfo
{
    ...
    //登录成功，零云用户信息
    public static function lingyun($token)
    {
        $lingyun = \ThinkOauth::getInstance('lingyun', $token);
        $data    = $lingyun->call('index/get_user_info');
        if ($data['success'] === 1) {
            $userInfo['type'] = 'LINGYUN';
            $userInfo['name'] = $data['username'];
            $userInfo['nick'] = $data['nickname'];
            $userInfo['head'] = $data['avatar_url'];
            return $userInfo;
        } else {
            throw_exception("获取零云用户信息失败：{$data['errmsg']}");
        }
    }
}
