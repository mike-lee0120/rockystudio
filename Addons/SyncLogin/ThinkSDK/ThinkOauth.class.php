...
abstract class ThinkOauth
{
    ...
    public function __construct($token = null, $config = null)
    {
        ...
        $config     = json_decode(D('Admin/Addon')->where(array('name' => 'SyncLogin'))->getField('config'), true);
    ...
    {
        $url = addons_url('SyncLogin://Login/callback', array('type' => strtolower($this->Type)));
        if (0 === strpos($url, 'http')) {
            $this->Callback = $url;
        } else {
            $this->Callback = C('TOP_HOME_PAGE') . $url;
        }
    }
